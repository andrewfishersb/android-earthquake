...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            JSONArray quakes = jsonObj.getJSONArray("features");

            for(int i = 0; i < quakes.length(); i++){
                JSONObject curQuake =  quakes.getJSONObject(i);
                JSONObject quakeProps = curQuake.getJSONObject("properties");
                location = quakeProps.getString("place");
                magnitude = quakeProps.getDouble("mag");
                time = quakeProps.getLong("time");
                String quakeURL = quakeProps.getString("url");
                earthquakes.add(new Earthquake(magnitude,location,time,quakeURL));
